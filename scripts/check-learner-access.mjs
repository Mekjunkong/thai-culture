...
for (const slug of ['week-1', 'week-2', 'week-3', 'week-4']) {
  assert.match(access, new RegExp(`slug: '${slug}'`), `allowlist is missing ${slug}`)
}
assert.equal((access.match(/slug: 'week-[1-4]'/g) ?? []).length, 4, 'allowlist must contain exactly four lesson slugs')
