      ...
      <main className="bg-paper text-tamarind">
          ...
          <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[1fr_0.9fr] lg:items-center">
            <div>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Link href="#products" className="inline-flex min-h-13 items-center justify-center rounded-none bg-ink px-7 py-4 font-bold text-surface transition hover:bg-ink/85 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-clay">
                  Compare products
                </Link>
                <a href="/products/onsite-chiang-mai-thai-lesson-pack.pdf" className="inline-flex min-h-13 items-center justify-center rounded-none border border-tamarind/15 bg-surface px-7 py-4 font-bold text-tamarind shadow-sm transition hover:border-clay focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-clay">
                  Download workbook
                </a>
              </div>
            </div>
            ...
            <aside className="rounded-none border border-tamarind/10 bg-surface p-5">
              ...
              <h2 className="mt-3 text-3xl font-serif font-normal leading-tight">One useful Thai sentence you can use today.</h2>
              <div className="mt-5 grid gap-3">
                {deliverables.slice(0, 4).map((item) => (
                  <div key={item} className="flex gap-3 rounded-none bg-jasmine p-4">
                    <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-sand text-xs font-bold text-surface">✓</span>
            ...
            <div className="mt-10 grid gap-4 lg:grid-cols-5">
              {productLadder.map((product) => (
                  <article key={product.name} className={`h-full rounded-none border p-5 shadow-sm transition duration-150 ease-out hover:-translate-y-0.5 ${product.name === 'Starter Pack' ? 'border-honey bg-sand/10' : 'border-tamarind/10 bg-jasmine'}`}>
                    ...
                    <h3 className="mt-3 text-2xl font-serif font-normal leading-tight">{product.name}</h3>
                    <p className="mt-3 text-xl font-bold text-clay">{product.price}</p>
        ...
        <section className="px-4 py-16 md:py-20">
          <div className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-2">
            <article className="rounded-none border border-tamarind/10 bg-surface p-6 shadow-sm">
              <p className="text-sm font-bold text-clay">Online product</p>
              <h2 className="mt-3 text-4xl font-serif font-normal leading-tight tracking-[-0.04em]">Remote lessons that still feel personal.</h2>
              <div className="mt-6 grid gap-3">
                {onlineFlow.map(([step, detail]) => (
                  <div key={step} className="rounded-none bg-jasmine p-4">
                    <p className="font-bold text-clay">{step}</p>
        ...
        <section className="bg-ink px-4 py-16 text-surface md:py-20">
          <div className="mx-auto max-w-6xl">
            ...
            <h2 className="mt-3 max-w-3xl text-4xl font-serif font-normal leading-tight tracking-[-0.04em] md:text-5xl">A real beginner package, not random lessons.</h2>
            <div className="mt-10 grid gap-3 md:grid-cols-5">
              {curriculum.map((item) => (
                  <article key={item.lesson} className="rounded-none border border-surface/10 bg-surface/10 p-5">
                    <p className="text-sm font-bold text-honey">{item.lesson}</p>
                    <h3 className="mt-3 text-xl font-serif font-normal leading-tight">{item.focus}</h3>
        ...
        <section id="book" className="px-4 py-16 md:py-20">
          <div className="mx-auto max-w-4xl rounded-none bg-surface p-6 text-center md:p-10">
            <p className="text-sm font-bold text-clay">Best next step</p>
            <h2 className="mt-3 text-4xl font-serif font-normal leading-tight tracking-[-0.04em] md:text-5xl">Start with one trial or one Starter Pack.</h2>
            ...
            <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
              <Link href="/book" className="inline-flex min-h-12 items-center justify-center rounded-none bg-honey px-6 py-3 font-bold text-tamarind transition hover:opacity-85 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-clay">
                Book a lesson
              </Link>
              <Link href="/practice" className="inline-flex min-h-12 items-center justify-center rounded-none border border-tamarind/15 px-6 py-3 font-bold text-clay transition hover:border-clay focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-clay">
