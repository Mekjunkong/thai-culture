    ...
    <>
      ...
      <main className="min-h-screen bg-jasmine px-4 py-12 text-tamarind md:py-18">
        <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[0.92fr_1.08fr] lg:items-start">
          <section className="rounded-[2rem] bg-ink p-6 text-surface shadow-2xl shadow-ink/20 md:p-8">
            <p className="text-sm font-black uppercase tracking-[0.16em] text-honey">Course account</p>
            ...
            <ul className="mt-8 grid gap-3 text-sm leading-6 text-surface/82">
              {['Email/password login', 'Persistent browser session', 'Save your progress on this device', 'Easy course access after purchase'].map((item) => (
                <li key={item} className="flex gap-3 rounded-2xl border border-surface/10 bg-surface/8 p-3">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-honey text-xs font-black text-tamarind" aria-hidden="true">✓</span>
                  <span>{item}</span>
                </li>
              ))}
            </ul>
